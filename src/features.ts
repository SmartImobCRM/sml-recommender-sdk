...
export const get_dummies = (imoveis:ImovelInput[]) => {
    ...
    for (const imovel of imoveis) {
        if (imovel.cidade_id && 
            typeof imovel.cidade_id === 'string'
        ) possible_vars.add(`cidade_id_${imovel.cidade_id}`);

        if (imovel.estado_id && 
            typeof imovel.estado_id === 'string'
        ) possible_vars.add(`estado_id_${imovel.estado_id}`);
    ...
    const imoveis_with_dummies = imoveis.map<ImovelInputWithDummies>(imovel => {
        let imovel_with_dummies:ImovelInputWithDummies = {...imovel};
...
export const weight_by_dummy_names = (dummy_names:string[], sample_size:number):number[] => dummy_names.map(name => {
